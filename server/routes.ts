...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      // Check if user is using a temporary password (generated by the system)
      // Temporary passwords are 12-16 characters with mixed case and numbers
      const isTemporaryPassword = /^[A-Z][a-z]+[0-9]+[!@#$%^&*]?$/.test(password) && password.length >= 12;
